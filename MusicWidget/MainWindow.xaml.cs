    ...
    public partial class MainWindow : Window
    {
        ...
        public void ApplyConfig()
        {
            ...
            if (hwnd != IntPtr.Zero)
            {
                ...
                if (AppConfig.Current.IsClickThrough)
        ...
        public void ForceRefreshColors()
        {
        ...
        public void ToggleBgFade()
        {
            ...
            if (enable) BlurBgImage.Opacity = 0.55;
            double targetRadius = enable ? 1.5 : 0.0;

            var anim = new DoubleAnimation(targetRadius, TimeSpan.FromMilliseconds(750))
            {
            ...
            if (!enable)
            {
                anim.Completed += (s, e) => BlurBgImage.Opacity = 0;
            }
        ...
        private void SetupTrayIcon()
        {
            ...
            unlockItem.Click += (s, e) =>
            {
                AppConfig.Current.IsClickThrough = false;
                AppConfig.Save();
                ApplyConfig();
        ...
        private async void InitSMTC()
        {
            ...
            _sessionManager.CurrentSessionChanged += (s, e) => UpdateSession();
            UpdateSession();
        }
        ...
        private void Session_PlaybackInfoChanged(GlobalSystemMediaTransportControlsSession sender, PlaybackInfoChangedEventArgs args)
        {
            Dispatcher.Invoke(() =>
            {
                ...
                if (PlayPauseIcon.Kind != newKind)
                {
                    ...
                    {
                        PlayPauseIcon.Kind = newKind;
                        var fadeIn = new DoubleAnimation(1, TimeSpan.FromMilliseconds(120));
                        PlayPauseIcon.BeginAnimation(UIElement.OpacityProperty, fadeIn);
                    };
        ...
        private async void RefreshMediaInfo()
        {
            ...
            {
                if (albumArtist.Contains(artist) || albumArtist.Contains("/") || albumArtist.Contains(","))
                    displayArtist = albumArtist;
                else if (!artist.Contains(albumArtist))
            ...
            await Dispatcher.InvokeAsync(async () =>
            {
                ...
                if (!(TrackInfoContainer.RenderTransform is TranslateTransform))
                    TrackInfoContainer.RenderTransform = new TranslateTransform();
                var translate = (TranslateTransform)TrackInfoContainer.RenderTransform;

                double targetBlurOpacity = AppConfig.Current.EnableBgFade ? 0.45 : 0.0;
                ...
                else
                {
                    ...
                    var info = _currentSession.GetPlaybackInfo();
                    PlayPauseIcon.Kind = info.PlaybackStatus == GlobalSystemMediaTransportControlsSessionPlaybackStatus.Playing ? PackIconKind.Pause : PackIconKind.Play;
                    ApplyColorsAndLaunchColorEngine(palette);
        ...
        private void SetupParallaxAndSpotlight(FrameworkElement container, FrameworkElement backgroundLayer, FrameworkElement foregroundLayer, System.Windows.Shapes.Rectangle spotlight)
        {
            ...
            var bgScale = new ScaleTransform(1, 1);
            var bgTranslate = new TranslateTransform(0, 0);
            bgTransformGroup.Children.Add(bgScale);
            bgTransformGroup.Children.Add(bgTranslate);
            backgroundLayer.RenderTransformOrigin = new Point(0.5, 0.5);
            backgroundLayer.RenderTransform = bgTransformGroup;

            var fgTransformGroup = new TransformGroup();
            var fgScale = new ScaleTransform(1, 1);
            var fgTranslate = new TranslateTransform(0, 0);
            fgTransformGroup.Children.Add(fgScale);
            fgTransformGroup.Children.Add(fgTranslate);
            foregroundLayer.RenderTransformOrigin = new Point(0.5, 0.5);
            foregroundLayer.RenderTransform = fgTransformGroup;

            if (spotlight.Fill is RadialGradientBrush brush)
            {
                container.PreviewMouseLeftButtonDown += (s, e) =>
                {
                    var shrink = new DoubleAnimation(0.92, TimeSpan.FromMilliseconds(100)) { EasingFunction = new CubicEase { EasingMode = EasingMode.EaseOut } };
                    bgScale.BeginAnimation(ScaleTransform.ScaleXProperty, shrink);
                    bgScale.BeginAnimation(ScaleTransform.ScaleYProperty, shrink);
                    fgScale.BeginAnimation(ScaleTransform.ScaleXProperty, shrink);
                    fgScale.BeginAnimation(ScaleTransform.ScaleYProperty, shrink);
                };

                container.PreviewMouseLeftButtonUp += (s, e) =>
                {
                    var bounce = new DoubleAnimation(1.05, TimeSpan.FromMilliseconds(400)) { EasingFunction = new BackEase { EasingMode = EasingMode.EaseOut, Amplitude = 2.5 } };
                    var fgBounce = new DoubleAnimation(1.15, TimeSpan.FromMilliseconds(400)) { EasingFunction = new BackEase { EasingMode = EasingMode.EaseOut, Amplitude = 2.5 } };

                    bgScale.BeginAnimation(ScaleTransform.ScaleXProperty, bounce);
                    bgScale.BeginAnimation(ScaleTransform.ScaleYProperty, bounce);
                    fgScale.BeginAnimation(ScaleTransform.ScaleXProperty, fgBounce);
                    fgScale.BeginAnimation(ScaleTransform.ScaleYProperty, fgBounce);
                };

                container.MouseEnter += (s, e) =>
                {
                    bgTranslate.BeginAnimation(TranslateTransform.XProperty, null);
                    bgTranslate.BeginAnimation(TranslateTransform.YProperty, null);
                    fgTranslate.BeginAnimation(TranslateTransform.XProperty, null);
                    fgTranslate.BeginAnimation(TranslateTransform.YProperty, null);

                    bgScale.BeginAnimation(ScaleTransform.ScaleXProperty, new DoubleAnimation(1.05, TimeSpan.FromMilliseconds(150)));
                    bgScale.BeginAnimation(ScaleTransform.ScaleYProperty, new DoubleAnimation(1.05, TimeSpan.FromMilliseconds(150)));

                    fgScale.BeginAnimation(ScaleTransform.ScaleXProperty, new DoubleAnimation(1.15, TimeSpan.FromMilliseconds(150)));
                    fgScale.BeginAnimation(ScaleTransform.ScaleYProperty, new DoubleAnimation(1.15, TimeSpan.FromMilliseconds(150)));

                    spotlight.BeginAnimation(UIElement.OpacityProperty, new DoubleAnimation(1, TimeSpan.FromMilliseconds(150)));
                };
                ...
                container.PreviewMouseMove += (s, e) =>
                {
                    ...
                    double offsetX = (pos.X - w / 2) / (w / 2);
                    double offsetY = (pos.Y - h / 2) / (h / 2);

                    brush.Center = new Point(pos.X / w, pos.Y / h);
                    brush.GradientOrigin = new Point(pos.X / w, pos.Y / h);

                    bgTranslate.X = offsetX * 4;
                    bgTranslate.Y = offsetY * 4;
                    fgTranslate.X = offsetX * 12;
                    fgTranslate.Y = offsetY * 12;
                };

                container.MouseLeave += (s, e) =>
                {
                    var smoothReset = new DoubleAnimation(0, TimeSpan.FromMilliseconds(250)) { EasingFunction = new CubicEase { EasingMode = EasingMode.EaseOut } };
                    var scaleReset = new DoubleAnimation(1.0, TimeSpan.FromMilliseconds(250)) { EasingFunction = new CubicEase { EasingMode = EasingMode.EaseOut } };

                    bgScale.BeginAnimation(ScaleTransform.ScaleXProperty, scaleReset);
                    bgScale.BeginAnimation(ScaleTransform.ScaleYProperty, scaleReset);
                    bgTranslate.BeginAnimation(TranslateTransform.XProperty, smoothReset);
                    bgTranslate.BeginAnimation(TranslateTransform.YProperty, smoothReset);

                    fgScale.BeginAnimation(ScaleTransform.ScaleXProperty, scaleReset);
                    fgScale.BeginAnimation(ScaleTransform.ScaleYProperty, scaleReset);
                    fgTranslate.BeginAnimation(TranslateTransform.XProperty, smoothReset);
                    fgTranslate.BeginAnimation(TranslateTransform.YProperty, smoothReset);

                    spotlight.BeginAnimation(UIElement.OpacityProperty, new DoubleAnimation(0, TimeSpan.FromMilliseconds(250)));
                };
            }
        }
        ...
        {
            _slideDirection = 1;
            if (PrevIcon.RenderTransform is TransformGroup tg && tg.Children[1] is TranslateTransform trans)
            {
                var bounce = new DoubleAnimation(-20, 0, TimeSpan.FromMilliseconds(500))
                { EasingFunction = new ElasticEase { EasingMode = EasingMode.EaseOut, Oscillations = 1, Springiness = 4 } };
                trans.BeginAnimation(TranslateTransform.XProperty, bounce);
            }
        ...
        {
            _slideDirection = -1;
            if (NextIcon.RenderTransform is TransformGroup tg && tg.Children[1] is TranslateTransform trans)
            {
                var bounce = new DoubleAnimation(20, 0, TimeSpan.FromMilliseconds(500))
                { EasingFunction = new ElasticEase { EasingMode = EasingMode.EaseOut, Oscillations = 1, Springiness = 4 } };
                trans.BeginAnimation(TranslateTransform.XProperty, bounce);
